...
export default function BackgroundMusic() {
  ...
  const changeTrack = (direction: 'next' | 'prev') => {
    ...
    localStorage.setItem('bg-music-track', newIndex.toString());
  };
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="text-sm font-black text-white truncate leading-tight uppercase font-mono tracking-wide bg-gradient-to-r from-[#00c8ff] to-white bg-clip-text text-transparent">
                      {currentTrack.title}
                    </h4>
                    <p className="text-xs text-neutral-400 truncate leading-snug">{currentTrack.artist}</p>
                  </div>
                  <div className="flex gap-1 mt-1">
                    <button onClick={() => changeTrack('prev')} className="text-neutral-500 hover:text-white p-0.5 rounded transition-colors" title="Previous Track">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="19 20 9 12 19 4 19 20"/><line x1="5" y1="19" x2="5" y2="5" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                    </button>
                    <button onClick={() => changeTrack('next')} className="text-neutral-500 hover:text-white p-0.5 rounded transition-colors" title="Next Track">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 4 15 12 5 20 5 4"/><line x1="19" y1="5" x2="19" y2="19" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                    </button>
                  </div>
                </div>
                <span className="text-[9px] text-neutral-500 font-medium tracking-tight mt-0.5 truncate">Source: {currentTrack.source}</span>
              </div>
            </div>
